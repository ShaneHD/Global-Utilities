...
public class PacketHandler {
	...
	public PacketHandler(NetworkHelper out) {
		this.out = out;
	}

	private void error(String command, int argsSize, Throwable t) {
		t.printStackTrace();
		Log.get().warning("Command " + StringUtils.quote(command) + " failed with args size " + StringUtils.quote(argsSize) + "; INFO= " + StringUtils.format(t));
	}
